...
**Date:** 2026-03-20
**Outcome:** Completed. All 112 tests passing. All four re-audits cleared.

### Summary of Work
- Added `vocabularies` and `preferences` tables to `src/common/schema.ts` (auto-migrated via `CREATE TABLE IF NOT EXISTS`)
- Seeded 92 entries across 6 fields (metal, denomination, grade, era, die_axis, mint) with numismatically-weighted `usage_count` values
- Added 5 Zod schemas (`VocabGetSchema`, `VocabAddSchema`, `VocabSearchSchema`, `VocabIncrementSchema`, `VocabResetSchema`) with `.strict()` and defence-in-depth `ALLOWED_VOCAB_FIELDS` allowlist
- Implemented 6 DB service methods with parameterized queries and redundant runtime allowlist guard
- Registered 5 vocab IPC handlers; `seedVocabularies()` wired to `app.whenReady()` (not on IPC surface)
- Created `AutocompleteField` component (ARIA 1.2 combobox, keyboard nav, scroll-close, add-new, reset)
- Created `useVocabularies` hook with module-level cache and fire-and-forget usage increment
- Updated `LedgerForm` — 6 fields (era, mint, denomination, die_axis, metal, grade) replaced with `AutocompleteField`
- 75 new test cases across 3 files; `@vitest/coverage-v8` installed with coverage config

### Pain Points
- Module-level cache in `useVocabularies` requires an exported `clearVocabCache()` function for test isolation — documented in AGENTS.md
- `validation.ts` coverage doesn't appear in the Vitest report because `src/common/` is outside the Vite `root: 'src/renderer'`; coverage is verified by the 27 explicit branch-targeting test cases instead
- The `opacity 0.15s ease` transition on `.autocomplete-dropdown` is a no-op because `display: none/block` toggling is not transitionable (noted by `curating-ui`; acceptable for Phase 6a)
...
- [Future Phase 6b: Use Nomisma.org SPARQL to populate `locale = 'es'` vocabulary rows — see Phase 6b blueprint note]
- [Future: Replace `display: none/block` dropdown toggle with `visibility: hidden` + `pointer-events: none` to make the opacity fade transition functional]
- **Core Doc Revision:** `AGENTS.md` updated with Vocabulary System patterns (§ above Database & File System)
