...
**Date:** 2026-03-22
**Outcome:** Completed — all 280 suite tests pass, `npx tsc --noEmit` clean, no regressions.

### Summary of Work
- Added `year_numeric` as 7th item in metrics grid (`LedgerForm.tsx`) with `type="number"`, `?? ''` null guard, and `parseInt`/`isNaN` double guard on `onChange`.
- Relabelled `year_display` from "Year" → "Date" (EN) / "Fecha" (ES) via i18n value change only — no structural code change.
- Added `ledger.yearCe` and `ledger.placeholders.yearCe` to both `en.json` and `es.json`; i18n parity test enforces it automatically.
- Corrected `glossaryFields.ts` `era` entry: description now reflects historical period classifier role, vocabulary table replaced with 11-row period-range reference.
- Fixed `seed_data.ts`: `era` type widened to `string`, Edward I Penny corrected to `'High Medieval'`, Hadrian Denarius added as second entry (chronologically ordered).
- 7 new tests added to `LedgerForm.test.tsx` covering all `onChange` branches, null/undefined initial state, and glossary-hint click.

### Pain Points
- None. The `?? ''` vs `|| ''` distinction (year 0 validity) was the only subtlety worth documenting — already captured in the blueprint and code comment.
