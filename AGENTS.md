...
- **Bulk Write Pattern:** When performing multiple write operations against the same IPC channel in a loop (e.g. bulk edit), use sequential `await` calls — not `Promise.all`. `better-sqlite3` serialises writes on the Main-process IPC queue, and sequential calls give cleaner per-call error attribution. If any call fails, collect the error and continue the loop; surface a summary result (e.g. "Updated N coins. E errors.") rather than aborting the batch. When a bulk write bypasses a hook and calls `window.electronAPI` directly, the caller is responsible for triggering a `refresh()` on completion.
- **Scoped Export Pattern:** `export:toZip` and `export:toPdf` accept an optional `coinIds?: number[]` parameter (Zod-validated, max 5000). Passing `coinIds` scopes the export to those coins; omitting it exports the full collection. This is the canonical pattern for any future filtered export operation.
- **Two-Phase Import Pattern (CAB-C):** Any Main-side operation requiring a native file dialog followed by processing uses a staged-path design. Phase 1 (`import:zipPreview`): open `dialog.showOpenDialog` in Main, cache the absolute path in a module-level `let stagedZipPath: string | null` variable, return only metadata to the renderer. Phase 2 (`import:zipExecute`): renderer sends validated options (never a path); Main reads from the staged path and clears it in a `finally` block. `import:cancel` clears `stagedZipPath`. This ensures the renderer never supplies or receives a file path. See `src/main/import/` for reference implementations and `docs/reference/ipc_api.md` for the handler API. Note: CSV import was removed post-release (see CAB-C §11); `src/main/import/csv.ts` is an internal module used only by the ZIP importer.
...
- **Managed Fields:** Seven coin fields use the vocabulary system: `metal`, `denomination`, `grade`, `era`, `die_axis`, `mint`, `rarity`. Valid field names are the single source of truth in `ALLOWED_VOCAB_FIELDS` in `src/common/validation.ts`.
...
- **AutocompleteField Pattern:** Vocab-backed form fields use the `AutocompleteField` component + `useVocabularies(field)` hook. The hook holds a module-level cache keyed as `"${field}:${locale}"` — always include locale in the cache key to prevent cross-locale stale hits. Call `clearVocabCache()` (exported for tests only) in `beforeEach` to prevent cross-test contamination. Usage increment (`incrementUsage`) is fire-and-forget — never block the UI on it. **Wiring:** inject usage tracking via the optional `onIncrementUsage` prop (e.g. `onIncrementUsage={eraVocab.incrementUsage}` in LedgerForm) — the component never calls `incrementVocabUsage` directly. The prop fires only for existing options (`selectOption`), never for new entries (`handleAddNew`). Suggestion re-ordering is deferred to the next cache miss (next navigation to the form) — not immediate. **Testing multiple dropdowns:** when a form renders several `AutocompleteField` instances simultaneously, use `within(container.querySelector('.autocomplete-field'))` to scope queries — plain `getByRole('option', { name })` will find multiple elements and throw. Flush the async `getVocab` call before opening any dropdown with `await act(async () => { await new Promise(r => setTimeout(r, 0)); })`.
...
- **Validation Mandate:** Every feature must be verified with its corresponding automated test.
  ...
  - **Testing Standards:** Refer to `docs/guides/testing_standards.md` for guidelines on the **Colocation Rule** and mocking patterns. Main-process handler tests live in `src/main/__tests__/` and test the extracted pure async functions (e.g. `processCsvImport`) directly with mocked `dbService` — do not unit-test `ipcMain.handle` registrations (that requires an Electron environment).
